...
export class MeasureConfigurator implements DataQueryConfigurator, ChartConfigurator {
  ...
  private readonly debouncedLoad = new DebouncedTask(taskHandle => this.loadMetadata(taskHandle))
  ...
  constructor(private readonly serverConfigurator: ServerConfigurator,
    ...
    if (this.parent != null) {
      watch(this.parent.value, this.debouncedLoad.executeFunctionReference)
    }
  }
  ...
  configureQuery(query: DataQuery, configuration: DataQueryExecutorConfiguration): boolean {
    ...
    // if (parent != null) {
    //   const parentFilterValues = toArray(parent.value.value)
    //   if (parentFilterValues.length === 0) {
    //     return false
    //   }
    //
    //   // add parent dimension field to result to create series for each combination of parent dimension and measure
    //   const filter = {field: parent.name, value: parentFilterValues[0]}
    //   query.addFilter(filter)
    //   let parentFilterValueIndex = 1
    //   configuration.extraQueryMutator = _ => {
    //     filter.value = parentFilterValues[parentFilterValueIndex++]
    //     return parentFilterValueIndex !== parentFilterValues.length
    //   }
    // }
    return true
  }
