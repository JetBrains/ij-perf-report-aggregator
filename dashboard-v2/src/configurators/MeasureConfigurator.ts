...
export class MeasureConfigurator implements DataQueryConfigurator {
  ...
  configure(query: DataQuery, configuration: DataQueryExecutorConfiguration): boolean {
    ...
    configureQuery(values, query, configuration, this.skipZeroValues)
    return true
  }
...
export class PredefinedMeasureConfigurator implements DataQueryConfigurator {
  constructor(private readonly measures: Array<string>,
              public skipZeroValues: boolean = true) {
  }

  configure(query: DataQuery, configuration: DataQueryExecutorConfiguration): boolean {
    configureQuery(this.measures, query, configuration, this.skipZeroValues)
    return true
  }
}

function configureQuery(values: Array<string>,
                        query: DataQuery,
                        configuration: DataQueryExecutorConfiguration,
                        skipZeroValues: boolean): void {
