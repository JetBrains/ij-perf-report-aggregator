...
export class ActivityChartManager extends XYChartManager {
  ...
  // isUseYForName - if true, names are more readable, but not possible to see all components
  // because layout from top to bottom (so, opposite from left to right some data can be out of current screen)
  ...
  private get nameAxis(): CategoryAxis {
    return this.chart.xAxes.getIndex(0) as CategoryAxis
  }

  private configureNameAxis(): void {
    const nameAxis = this.chart.xAxes.push(new CategoryAxis())
  ...
  protected configureDurationAxis(): DurationAxis {
    const durationAxis = this.chart.yAxes.push(new DurationAxis())
