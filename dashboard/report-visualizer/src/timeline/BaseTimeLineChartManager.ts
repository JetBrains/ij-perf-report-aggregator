...
export abstract class BaseTimeLineChartManager extends XYChartManager {
  ...
  private readonly threadFirstRowIndexSet = new Map<number, Color>()
  ...
  private addHeightAdjuster(levelAxis: Axis) {
  ...
  protected configureLevelAxis(): CategoryAxis {
    const levelAxis = this.chart.yAxes.push(new CategoryAxis())
    ...
    levelAxis.renderer.labels.template.fill = color("#ff0000")
  ...
  protected configureSeries(valueLabelText: string): void {
  ...
  private getItemByColumn(column: Sprite): TimeLineItem | null {
  ...
  protected configureDurationAxis(): DurationAxis {
    const durationAxis = this.chart.xAxes.push(new DurationAxis())
  ...
  protected transformParallelToTimeLineItems(originalItems: Array<ItemV0>, colorSet: ColorSet, threadRowFirstIndex: number): Array<TimeLineItem> {
  ...
  protected computeRangeMarkers(dataManager: DataManager): void {
    this.collectGuides(dataManager, this.guides)
    this.applyGuides()
  }

  // final
  protected applyGuides(): void {
    const nameAxis = this.chart.xAxes.getIndex(0) as DurationAxis
