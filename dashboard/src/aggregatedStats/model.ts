...
export interface MetricInfo {
  ...
  readonly isInstant: boolean | undefined
}
...
export function getFilters(request: DataRequest): Array<DataQueryFilter> {
  ...
  result.push({field: "project", value: request.projects})
...
export function encodeQueries(queries: Array<DataQuery>): string {
  console.debug("data-query: " + JSON.stringify(queries, null, 2))
  return rison.encode(queries)
}
