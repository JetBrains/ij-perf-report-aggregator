...
func NewInsertReportManager(db *sqlx.DB, dbName string, context context.Context, tableName string, insertWorkerCount int, logger *zap.Logger) (*InsertReportManager, error) {
	...
	sb.WriteString("machine, build_time, generated_time, project, tc_build_id, ")
	if config.HasInstallerField {
		sb.WriteString("tc_installer_build_id, ")
	}
	sb.WriteString("tc_build_properties, branch, raw_report")
	if config.HasInstallerField {
		sb.WriteString(", build_c1, build_c2, build_c3")
	}
...
func (t *InsertReportManager) WriteMetrics(product string, row *RunResult, branch string, providedProject string, logger *zap.Logger) error {
	...
	if t.config.HasInstallerField {
		args = append(args, row.TcInstallerBuildId)
	}
	args = append(args, row.TcBuildProperties, branch, row.RawReport)
	if t.config.HasInstallerField {
		args = append(args, row.BuildC1, row.BuildC2, row.BuildC3)
	}
